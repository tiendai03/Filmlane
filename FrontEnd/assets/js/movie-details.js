document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!movieId) {
        ...
        fetch('products.json')
            ...
            .then(data => {
                const tvSeriesData = data.tvSeriesDatas;
                const modifiedUrls = [];
                ...
                tvSeriesData.forEach(ser => {
                    ...
                    modifiedUrls.push(newUrl.href); // Lưu trữ URL đã được sửa đổi vào mảng modifiedUrls
    ...
    fetch(`http://localhost:3000/tvSeriesDatas/${movieId}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
