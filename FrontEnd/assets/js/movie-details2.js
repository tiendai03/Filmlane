document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!movieId) {
        ...
        fetch('products.json')
            ...
            .then(data => {
                const tvSeriesData = data.tvSeriesDatass;
                const modifiedUrlss = [];
                ...
                tvSeriesData.forEach(ser => {
                    ...
                    modifiedUrlss.push(newUrl.href); // Lưu trữ URL đã được sửa đổi vào mảng modifiedUrls
                ...
                console.log('Modified URLs:', modifiedUrlss);
    ...
    fetch(`http://localhost:3000/tvSeriesDatass/${movieId}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
